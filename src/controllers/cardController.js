...
class CardController {
  ...
  async createCard(req, res) {
    try {
      ...
      if (!name || !rarity || !attackPoints || !defensePoints || !collectionId) {
        return res.status(400).json({ error: "Os campos Nome, Raridade, Pontos de Ataque, Pontos de Defsa e ID da Coleção são obrigatórios" });
      }
      ...
      if (!newCard) {
        return res.status(400).json({ error: "Erro ao criar nova carta" });
      }
  ...
  async deleteCard(req, res) {
    try {
      ...
      res.status(200).json({error: "Carta não encontrada"});
    } catch (error) {
